...
```bash
curl -X POST https://gateway.pipeworx.io/v1/tools/stat_ee_find_table \
  -H 'Content-Type: application/json' \
  -d '{"query":"average monthly wage","fetch_latest":true}'
```

No account needed for the first calls. Inspect any tool: `GET https://gateway.pipeworx.io/v1/tools/stat_ee_find_table`. Find one: `POST https://gateway.pipeworx.io/v1/tools/search_packs` with `{"query":"..."}`.
